...
comm = df.mpi_comm_world() # MPI = message passing interface --> parallel computing allowed
#MPI class #(object):
#mpi_comm_world - time stamp?
nx = 10
mesh = df.UnitSquareMesh(nx, nx)
params = poro.ParamParser() #  function used to parse params?
data_dir = str(uuid.uuid4())
# Universal Unique Identifier, 128 bit number random, identifictaion
...
#boundaries applied to MeshFunction
#meshfunction also allows storage of subdomains
# argument specifying the type is "size_t" --> meaning? unsigned integer type of at least 16 bits
#mesh topology creates empty mesh topology
# mesh entity is identified globally as a pair with dim
#dim - topological dimension , with i index of topoligcal dimension
# - 1 ??
...
def set_xdmf_parameters(f):
    f.parameters['flush_output'] = True #preview of a file
    f.parameters['functions_share_mesh'] = True
    f.parameters['rewrite_function_mesh'] = False  #Controls whether mesh will be rewritten every timestep

    # rewrite_function_mesh:Controls whether all functions on a single time
    # step share the same mesh; for time dependent functions where mesh does not change, when true -->
...
dx = df.Measure("dx") #integral function - why required
